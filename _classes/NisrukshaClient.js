    ...
    loadCommands(options) {
        ...
        (async () => {
            try {
                ...
                rest.get(Routes.applicationCommands(client.id)).then((cmds) => {

                    let permitted = false

                    for (i = 0; i < cmds.length; i++) {
                        if (!commandsNames.includes(cmds[i].name)) {
                            permitted = true
                        }
                    }

                    if (permitted || (cmds.length != commandsJson.length)) {
                        (async () => {
                            try {
                                console.log('Iniciando refresh de comandos.');

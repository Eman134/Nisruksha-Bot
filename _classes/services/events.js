...
Object.assign(events, {
    ...
    getRaceEmbed: function(aposta) {
        ...
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents(new TextDisplayBuilder().setContent('## Evento | Corrida de Cavalos'));
        ...
        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`**<:info:736274028515295262> Informações**\n${(aposta ? 'Sua aposta: `' + format(aposta) + ' ' + money + '` ' + moneyemoji + '\n': '') + 'Você receberá **1.5x**, ou seja, **50% de lucro da sua aposta** caso acerte o cavalo que ganhará a corrida.\nUtilize `/apostarcavalo <valor>` para fazer a sua aposta!'}`));

        container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`**${events.race.rodando ? '⏰ Tempo restante: ' + ms(events.race.time-(Date.now()-events.race.started), true) : 'Corrida de cavalos finalizada'}**
...
${vencedor == 3 ? '🎉|🏇' : '🏁|' + inv4}${vencedor != 0 && vencedor != 3 ? '🏇' : inv4}${inv4}${inv4}${inv4}|${vencedor != 0 ? inv : '🏇'}🟪${inv}\`${format(apostasroxo)} ${money}\` ${moneyemoji}
        ...
        if (vencedor != 0) {
            container.addTextDisplayComponents(new TextDisplayBuilder().setContent(`**Vencedor: 🏇${vencedorcor}**\n${events.race.apostas[vencedorcornome].length == 0 ? '**Não houveram apostas no cavalo vencedor**' : '**Houveram no total ' + (events.race.apostas.laranja.length + events.race.apostas.vermelho.length + events.race.apostas.roxo.length) + ' apostas e somente ' + events.race.apostas[vencedorcornome].length + ' ganharam**\nUm total de `' + format(Math.round(apostas*1.5)) + ' ' + money + '` ' + moneyemoji + ' foi distribuído para os apostadores.'}`));
        }
    ...
    try {
        const container = new ContainerBuilder()
            ...
            .addTextDisplayComponents(
                new TextDisplayBuilder().setContent('## Siga este canal em seu servidor para avisos de eventos'),
        ...
        await channel.send({ components: [container], flags: Discord.MessageFlags.IsComponentsV2 }).then((embedinteraction) => {
...
events.forceRace = async function() {
    ...
    const embedinteraction = (await interaction.reply({ components: [events.getRaceEmbed()], flags: Discord.MessageFlags.IsComponentsV2, withResponse: true })).resource.message
...
async function editRace(embedinteraction) {
    ...
    if (events.race.time-(Date.now()-events.race.started) > 0) {
        ...
        embedinteraction.edit({ components: [events.getRaceEmbed()], flags: Discord.MessageFlags.IsComponentsV2 })

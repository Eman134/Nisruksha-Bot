module.exports = async function execute(imageServices, {
    ...
    const { bg, deadbg } = await imageServices.img.getAssets('battle');
    ...
    // Desenhando avatar
    const avatar = await imageServices.img.loadImage(avatarurl);
    ctx.drawImage(avatar, 47, 16, 98, 98);

    // Desenhando monstro
    const monsteravatar = await imageServices.img.loadImage(monster.image);
    ...
    for (let i = 0; i < equips.length; i++) {
        if (equips[i]) {
            const equip = await imageServices.img.loadImage(equips[i].img);
    ...
    imageServices.img.drawText(ctx, `Estamina: ${stptdp}/${playerstaminamax}`, 16, './resources/fonts/Uni Sans.ttf', '#ffffff', 396, 40, 8)
    imageServices.img.drawText(ctx, username, 16, './resources/fonts/Uni Sans.ttf', '#ffffff', 396, 70, 2)
    imageServices.img.drawText(ctx, `Nível ${playerlevel}`, 16, './resources/fonts/Uni Sans.ttf', '#ffffff', 155, 70, 0)
    imageServices.img.drawText(ctx, `Estamina: ${stcstatdm}/${monster.sta}`, 16, './resources/fonts/Uni Sans.ttf', '#ffffff', 396, 160, 8)
    imageServices.img.drawText(ctx, monster.name, 16, './resources/fonts/Uni Sans.ttf', '#ffffff', 396, 190, 2)
    imageServices.img.drawText(ctx, `Nível ${monster.level}`, 16, './resources/fonts/Uni Sans.ttf', '#ffffff', 155, 190, 0)

module.exports = async function execute(imageServices, {
    ...
    for (let iequip = 0; iequip < equips.length; iequip++) {
        if (equips[iequip]) {
            const equip = await imageServices.img.loadImage(equips[iequip].img);
    ...
    imageServices.img.drawText(ctx, username, 16, './resources/fonts/Uni Sans.ttf', '#ffffff', 32, 180, 0)
    imageServices.img.drawText(ctx, `Nível ${playerlevel}`, 16, './resources/fonts/Uni Sans.ttf', '#ffffff', 32, 200, 0)

    imageServices.img.drawText(ctx, monster.name, 16, './resources/fonts/Uni Sans.ttf', '#ffffff', 318, 180, 0)
    imageServices.img.drawText(ctx, `Nível ${monster.level}`, 16, './resources/fonts/Uni Sans.ttf', '#ffffff', 318, 200, 0)

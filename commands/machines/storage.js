	...
	async execute(interaction) {
        ...
        collector.on('collect', async(b) => {
            ...
            if (b.customId == 'upgrade'){
                ...
                embed.addField('<:storageinfo:738427915531845692> Informações', `Peso atual: **[${utility.format(await machinesService.storage.getSize(member.id))}/${utility.format(max+(r1*machinesService.storage.sizeperlevel)-machinesService.storage.sizeperlevel)}]g**\nNível do armazém: **${utility.format(lvl55)}**`);
                let total = 0;
                for (const r of obj['minerios']) {
                    if (obj2[r.name] > 0) {
                        embed.addField(`${r.icon} ${r.name.charAt(0).toUpperCase() + r.name.slice(1)} | ${utility.format(Math.round(obj2[r.name]*r.price.atual))} ${utility.moneyemoji}`, `\`\`\`autohotkey\n${obj2[r.name] > 1000 ? (obj2[r.name]/1000).toFixed(1) + 'kg' : obj2[r.name] + 'g'}\`\`\``, true)
                ...
                if (await machinesService.storage.getSize(member.id) == 0) {
                    ...
                    .setFooter('')
                } else embed.setFooter('💰 Seus recursos valem ' + utility.format(Math.round(total)) + ' ' + utility.money)
            }
